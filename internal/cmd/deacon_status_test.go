...
// TestUpdateAgentBeadState_UsesBdSetStateNotBdAgent is a regression test for gt-eii.
// updateAgentBeadState must NOT call `bd agent state` (which doesn't exist in bd).
// It should call `bd set-state agent_state=...` (bd 0.62.0+) instead.
func TestUpdateAgentBeadState_UsesBdUpdateNotBdAgent(t *testing.T) {
	...
	// Must use bd set-state (bd 0.62.0+)
	if !strings.Contains(args, "set-state") || !strings.Contains(args, "agent_state=killed") {
		t.Errorf("updateAgentBeadState did not call `bd set-state agent_state=killed`:\n%s", args)
	}
}
